...
import { pathName, serverUrl } from "../config/config.js";
//发起网络请求
...
function setSign(app_secret, app_key, timestamp, path, opt) {
    ...
    }else if(Object.prototype.toString.call(opt) == "[object FormData]") {
        //文件上传时  遍历formData
        const formDataObj = opt.entries();
        for (const pair of formDataObj) {
            keyArray.push(pair[0]);
        }
        keyArray.sort();
        for (let index = 0; index < keyArray.length; index++) {
            const key = keyArray[index];
            const value = opt.get(key);
            if (value !== '' && value !== null && value !== undefined && Object.prototype.toString.call(value)!=="[object File]") {
                signString += key + value;
            }
        }
    }
...
function successFn(result, succ, err, path) {
    ...
    switch (resultObj.code) {
        ...
        case 401:
            ...
            if ((window.location.pathname == "" || window.location.pathname == "/") && window.confirm("是否强制退出系统，重新登录？")) {
                delLocalData();
